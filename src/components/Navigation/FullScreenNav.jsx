...
const FullScreenNav = () => {
  ...
  const thumbA = '/images/menu-thumb.jpg'
  ...
  return (
    <div ref={fullScreenRef} id='fullscreennav' className='fullscreennav isolate fixed inset-0 z-50 hidden overflow-hidden bg-white text-black'>
      ...
      <div ref={fullNavLinksRef} className='relative flex h-dvh flex-col overflow-hidden'>
        <div className="navlink relative flex w-full justify-center p-5">
          <div className='w-[clamp(12rem,22vw,18rem)] cursor-pointer' onClick={() => go('/')}>
            <Logo fill='black' className='w-full h-auto' />
          </div>
          <button
            type='button'
          ...
          >
            <div className='absolute left-1/2 top-0 h-[clamp(12rem,16vw,18rem)] w-0.5 -rotate-45 origin-top bg-black transition-colors group-hover:bg-[#D9A99B] group-active:bg-[#D9A99B] lg:w-1'></div>
            <div className='absolute right-0 top-0 h-[clamp(12rem,16vw,18rem)] w-0.5 rotate-45 origin-top bg-black transition-colors group-hover:bg-[#D9A99B] group-active:bg-[#D9A99B] lg:w-1'></div>
          </button>
        </div>
        ...
        <div className='flex flex-1 flex-col items-center justify-center'>
          {links.map((item) => (
            <div
              key={item.to}
              className='link relative w-full origin-top cursor-pointer border-y border-black/20 text-center'
              onClick={() => go(item.to)}
            >
              <h1 className='font-[font2] text-[clamp(3.25rem,8vw,7rem)] text-center leading-[0.8] pt-3 uppercase'>
                {item.label}
              </h1>
              <div className='moveLink absolute inset-0 flex h-full w-full items-center overflow-hidden bg-[#D9A99B] text-black'>
